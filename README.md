...
crash relaunch; a fresh exact-package readiness handoff signed by the separately
trusted failover controller with relay-confirmed backup; OBS installation and
exact-input health; plus the remote public-egress observer. The owner-only JSON
report cannot be
...
preserve the controller report, signature, and trust root beside it. See
