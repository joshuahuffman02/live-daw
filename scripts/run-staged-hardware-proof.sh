...
if [[ -n "${host_readiness_report}" ]]; then
  ...
  report_failover_signature="$(/usr/bin/plutil -extract inputs.failoverReadinessSignaturePath raw -o - "${host_readiness_report}")"
  report_failover_trusted_signers="$(/usr/bin/plutil -extract inputs.failoverTrustedSignersPath raw -o - "${host_readiness_report}")"
  ...
  failover_signature_copy="${phase_directory}/external-failover-controller-readiness.json.sig"
  /bin/cp "${report_failover_signature}" "${failover_signature_copy}"
  /bin/chmod 600 "${failover_signature_copy}"
  /usr/bin/shasum -a 256 "${failover_signature_copy}" > "${failover_signature_copy}.sha256"
  /bin/chmod 600 "${failover_signature_copy}.sha256"
  failover_trust_copy="${phase_directory}/external-failover-controller-trusted-signers"
  /bin/cp "${report_failover_trusted_signers}" "${failover_trust_copy}"
  /bin/chmod 600 "${failover_trust_copy}"
  /usr/bin/shasum -a 256 "${failover_trust_copy}" > "${failover_trust_copy}.sha256"
  /bin/chmod 600 "${failover_trust_copy}.sha256"
