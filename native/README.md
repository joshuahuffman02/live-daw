...
17. Use `scripts/run-staged-hardware-proof.sh` for acceptance evidence. It refuses
    development/ad-hoc builds and cross-checks the signature-sealed source
    ...
    file hashes—including the signed readiness handoff from the separately trusted
    failover controller and its fresh relay-confirmed backup state—binds it to the
    requested phase/app/release metadata/route/profile/recording volume, and verifies
    at least two hours of continuous
